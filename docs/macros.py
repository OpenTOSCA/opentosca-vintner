...
"""
    ...
    def repo_link(path, title = None):
        return "[%(title)s](%(url)s){target=_blank}" % {"title": title if title else path, "url": "https://github.com/OpenTOSCA/opentosca-vintner/tree/main/" + path}

    @env.macro
    def tag_link(tag):
        return "[@%(tag)s](%(url)s){target=_blank}" % {"tag": tag, "url": "https://github.com/OpenTOSCA/opentosca-vintner/releases/tag/" + tag}

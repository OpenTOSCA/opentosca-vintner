...
async function main() {
    ...
    const vdmm_baseline_original = await stats(
        ...
        path.join(
            examplesDir,
            'unfurl-technology---boutique---baseline-original---only',
    ...
    const vdmm_plus_original_manual = await stats(
        ...
        path.join(
            examplesDir,
            'unfurl-technology---boutique---plus-original-manual---only',
            'variable-service-template.yaml'
        )
    )
    ...
    const vdmm_plus_original_automated_quality = await stats(
        ...
        path.join(
            examplesDir,
            'unfurl-technology---boutique---plus-original-automated---only',
            'variable-service-template.yaml'
        )
    )
    ...
    const vdmm_baseline_maintenance = await stats(
        ...
        path.join(
            examplesDir,
            'unfurl-technology---boutique---baseline-maintenance---only',
            'variable-service-template.yaml'
        )
    )
    ...
    const vdmm_plus_maintenance_manual = await stats(
        ...
        path.join(
            examplesDir,
            'unfurl-technology---boutique---plus-maintenance-manual---only',
            'variable-service-template.yaml'
        )
    )

...
export class Winery implements RepoPlugin {
    ...
    getAllTemplates(): {name: string; template: ServiceTemplate}[] {
        ...
        const searchPattern = path
            .join(this.templatePath, '/**/ServiceTemplate.tosca')
            .split(path.sep)
            .join(path.posix.sep)
    ...
    getTemplate(name: string): {name: string; template: ServiceTemplate} {
        const templatePath = path.resolve(path.join(this.templatePath, name, 'ServiceTemplate.tosca'))
    ...
    getRepo(): string {
        ...
        try {
            const wineryConfig = files.loadYAML(path.resolve(wineryConfigPath)) as {

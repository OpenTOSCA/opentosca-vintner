...
export function check(graph: Graph, options: ConsistencyOptions = {}) {
    if (options.disable_consistency_checks) return graph

    const relations = graph.relations.filter(relation => relation.present)
    const nodes = graph.nodes.filter(node => node.present)

    // Ensure that each relation source exists
    if (!options.disable_relation_source_consistency_check) {
        for (const relation of relations) {
            if (!graph.nodesMap.get(relation.source)?.present)
                throw new Error(`Relation source "${relation.source}" of relation "${relation.display}" does not exist`)
        }
    }

    // Ensure that each relation target exists
    if (!options.disable_relation_target_consistency_check) {
        for (const relation of relations) {
            if (!graph.nodesMap.get(relation.target)?.present)
                throw new Error(`Relation target "${relation.target}" of relation "${relation.display}" does not exist`)
        }
    }

    // Ensure that every component has at maximum one hosting relation
    if (!options.disable_ambiguous_hosting_consistency_check) {
    ...
    if (!options.disable_expected_hosting_consistency_check) {
        for (const node of nodes) {
            const relations = node.outgoing.filter(
    ...
    if (!options.disable_missing_artifact_parent_consistency_check) {
        const artifacts = graph.artifacts.filter(artifact => artifact.present)
    ...
    if (!options.disable_ambiguous_artifact_consistency_check) {
    ...
    if (!options.disable_missing_property_parent_consistency_check) {
        for (const property of graph.properties.filter(property => property.present)) {
    ...
    if (!options.disable_ambiguous_property_consistency_check) {
        for (const node of graph.nodes) {
            const names = new Set()
